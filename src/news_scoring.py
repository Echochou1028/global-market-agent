...
#    LOW_SCORE_TOP_N_PER_CATEGORY(当前=3，随实际反馈调整过)、
...
# 低权重新闻：每个分类的保留上限
#
# 曾经从10上调到15（应对分类从8类合并为3类的挤占效应），
# 但实测下来配合keep档一起，最终日报能到49条——开盘前
# 一小时根本看不完。用户明确反馈的是"能不能读完"而不是
# "分类会不会被挤占"，所以改成3——低权重池本来就是"次要
# 新闻，按分数择优"的排队区，3条足够覆盖每个分类里最值得看
# 的长尾新闻，真正重要的事件由keep档无条件保留，不受这个
# 上限影响。
# ============================================================

LOW_SCORE_TOP_N_PER_CATEGORY = 3
